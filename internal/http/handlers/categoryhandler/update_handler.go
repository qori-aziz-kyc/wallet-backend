...
func (handler *categoryHandler) UpdateHandler(ctx *gin.Context) {
	var categoryRequest request.CategoryRequest
	if err := ctx.ShouldBindUri(&categoryRequest); err != nil {
	...
	statusCode, category, err := handler.usecase.Update(ctx.Request.Context(), categoryRequest)
	if err != nil {
