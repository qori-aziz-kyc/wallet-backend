...
func (handler CategoryHandler) CreateHandler(ctx *gin.Context) {
	...
	statusCode, err := handler.usecase.Create(ctx.Request.Context(), categoryRequest)
	if err != nil {
		helper.NewErrorResponse(ctx, statusCode, err.Error())
		return
	}

	helper.NewSuccessResponse(ctx, statusCode, "create category success", map[string]interface{}{})
}
